            ...
            for ll=[0,1]
                for mm=[0,1]
                    for nn=[0,1]
                        ...
                        %from email
                        Ey=(1/8).*(-8.*u.*v.*(Cw+w)+u.^2.*(2.*Cw+w)+(2.*Cw+w).*(v.^2+w.^2)...
                            +4.*Cw.*u.*s+8.*Cu.*w.*s+6.*u.*w.*s+4.*w.*(-u.*(2.*Cu+u)+2.*Cw.*w...
                            +w.^2).*acoth(u./s)+4.*v.*(-2.*Cu.*w.*acoth(v./s)+2.*w.*(Cw+w).*atan(u./w)...
                            +(-u.*(2.*Cu+u)+2.*Cw.*w+w.^2).*atan((u.*v)./(w.*s)))-2.*(2.*Cw+w).*(v.^2+w.^2).*log(u+s)...
                            +8.*u.*v.*(Cw+w).*log(s-v));
                        ...
                        %Ey=(1/8)*((2.*Cw+w).*u.^2-8.*u.*v.*(Cw+w)+8.*u.*v.*(Cw+w).*log(s-v)...
                            ...
                            Ex=(1/8).*(-2.*Cw.*(u.^2-4.*u.*v+v.^2+w.^2+2.*v.*s)-...
                                w.*(u.^2-8.*u.*v+v.^2+w.^2+8.*Cv.*s+6.*v.*s)+...
                                4.*(2.*Cv.*u.*w.*acoth(u./s)+w.*(2.*Cv.*v+v.^2-w.*(2.*Cw+w)).*...
                                acoth(v./s)-u.*(2.*w.*(Cw+w).*atan(v./w)+(-v.*(2.*Cv+v)+2.*Cw.*w+w.^2).*atan((u.*v)./(w.*s))+...
                                2.*v.*(Cw+w).*log(s-u)))+2.*(2.*Cw+w).*(u.^2+w.^2).*log(v+s));
